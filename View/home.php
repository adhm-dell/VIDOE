...
$allVideos = $controller->getAllVideos();
$db = new DBController();
               ...
               <div class="video-block section-padding">
                  <div class="row">
                     ...
                     foreach ($allVideos as $video) {
                        $whereClause = '(id = ' . "'" . $video['category_id'] . "'" . ')';
                        // $categoryName = $db->select($whereClause, '', '', 'category');
                        $categoryName = "Movies";
                        $uploadDate = new DateTime($video['upload_date']);
                        $interval = $currentDate->diff($uploadDate);
                        $diff = $interval->format('%m months');
                        $image = "assets/Thumbnails/". basename($video['thumbnail']);

                        $html = <<<HTML
                           <div class="col-xl-3 col-sm-6 mb-3">
                              <div class="video-card">
                                 <div class="video-card-image">
                                    <a class="play-icon" href="{$video['file_path']}" ><i class="fas fa-play-circle"></i></a>
                                    <a href="#"><img class="img-fluid" src="$image" alt=""></a>
                                 </div>
                                    <div class="video-card-body">
                                       <div class="video-title">
                                       <a href="{$video['file_path']}">{$video['title']}</a>
                                    </div>
                                    <div class="video-page text-success">
                                       $categoryName <a title="" data-placement="top" data-toggle="tooltip" href="{$video['file_path']}" data-original-title="Verified"></a>
                                    </div>
                                    <div class="video-view">
                                       {$video['watches']} &nbsp;<i class="fas fa-calendar-alt"></i> $diff Months ago
                                    </div>
                                 </div>
                              </div>
                           </div>
